...
-   *For a large speech model,* set the `interim_results` to `true` in the JSON `start` message. `low_latency` is currently supported by `en-US` large speech models only. 
...
The `low_latency` parameter is available for most next-generation models and `en-US` large speech models. The parameter is not available with previous-generation models.
{: note}
