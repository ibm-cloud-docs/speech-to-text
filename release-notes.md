...
-   The HTTP `DELETE` request `/v1/sessions/{session_id}` does not return status code 415 "Unsupported Media Type." This return code has been removed from the documentation for the method.
...
-   The `GET /v1/sessions/{session_id}/observeResult` method is now named `/v1/sessions/{session_id}/observe_result`. The name `observeResult` is still supported for backward compatibility.
...
-   For HTTP `POST` requests to the `/v1/sessions` and `/v1/recognize` methods, error code 503 ("Service Unavailable") can be returned. The error code can also be returned when creating a WebSocket connection with the `recognize` method.
