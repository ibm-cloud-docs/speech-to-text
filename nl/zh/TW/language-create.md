...
1.  [訓練自訂語言模型](#trainModel-language)。將字組新增至自訂模型之後，您必須用這些字組來訓練模型。訓練會準備要用於語音辨識的自訂模型。模型必須經過訓練，才會使用新的或已修改的字組。
1.  訓練自訂模型之後，即可將其用於辨識要求。如果傳遞來進行轉錄的音訊中，包含自訂模型中定義的領域專用字組，則要求的結果會反映模型的加強詞彙。如需相關資訊，請參閱[使用自訂語言模型](/docs/services/speech-to-text/language-use.html)。

用來建立自訂語言模型的步驟會反覆進行。您可以新增語料庫、新增字組，並根據需要經常訓練或重新訓練模型。

您也可以將文法新增至自訂語言模型。文法會限制服務只能回應文法所辨識的那些字組。如需相關資訊，請參閱[將文法與自訂語言模型搭配使用](/docs/services/speech-to-text/grammar.html)。

語言模型自訂作業適用於大部分語言。如需相關資訊，請參閱[自訂作業的語言支援](/docs/services/speech-to-text/custom.html#languageSupport)。
{: note}
...
要由新自訂模型自訂的語言模型名稱。請指定 <code>GET /v1/models</code> 方法所傳回的其中一個支援語言模型。新模型只能搭配它自訂的基礎模型使用。</td>
  ...
  <tr>
    <td><code>dialect</code><br/><em>選用</em></td>
    <td style="text-align:center">字串</td>
    <td>
要與自訂模型搭配使用之指定語言的用語。依預設，用語符合基礎模型的語言；例如，美式英文語言模型 <code>en-US_BroadbandModel</code> 或 <code>en-US_NarrowbandModel</code> 的用語為 <code>en-US</code>。<br/></br>
此參數只對西班牙文模型有意義，服務會針對所指定用語的語音，為其建立適合的自訂模型：
      <ul style="margin-left:20px; padding:0px;">
        <li style="margin:10px 0px; line-height:120%;">
          <code>es-ES</code> 代表卡斯提亞西班牙文（預設值）
        </li>
        <li style="margin:10px 0px; line-height:120%;">
          <code>es-LA</code> 代表拉丁美洲西班牙文
        </li>
        <li style="margin:10px 0px; line-height:120%;">
          <code>es-US</code> 代表北美（墨西哥）西班牙文
        </li>
      </ul>
如果您指定用語，它必須是對基礎模型而言有效的。
    </td>
  </tr>
  <tr>
...
下列範例會建立名為 `Example model` 的新自訂語言模型。會針對基礎模型 `en-US-BroadbandModel` 建立模型，且具有說明 `Example custom language model`。`Content-Type` 標頭指定將 JSON 資料傳遞給方法。
...
此範例會傳回新模型的自訂作業 ID。每個自訂模型都以唯一的自訂作業 ID 來識別，此 ID 是一個廣域唯一 ID (GUID)。請使用與模型相關聯之呼叫的 `customization_id` 參數來指定自訂模型的 GUID。
...
新的自訂模型由使用其認證來建立該自訂模型的服務實例所擁有。如需相關資訊，請參閱[自訂模型的所有權](/docs/services/speech-to-text/custom.html#customOwner)。
...
-   如需使用語料庫的相關資訊，請參閱[使用語料庫](/docs/services/speech-to-text/language-resource.html#workingCorpora)。
-   如需服務如何將語料庫新增至模型的相關資訊，請參閱[新增語料庫檔案時會發生什麼情況？](/docs/services/speech-to-text/language-resource.html#parseCorpus)

語料庫會提供包含新字組的句子，以容許服務學習環境定義中的字組。然後，您可以個別擴增或修改模型的字組。相對於使用從語料庫新增的字組，只用個別字組來訓練模型較為耗時，且會產生較不具效益的結果。
{: tip}
...
-   使用 `corpus_name` 路徑參數指定語料庫的名稱。請使用符合自訂模型語言並反映語料庫內容的本地化名稱。
    -   名稱中最多可包含 128 個字元。
    -   請不要在名稱中包含空格、`/`（斜線）或 `\`（反斜線）。
    -   請不要使用已新增至自訂模型的語料庫名稱。
    -   請不要使用名稱 `user`，服務已保留此名稱來表示使用者所新增或修改的自訂字組。
-   將語料庫文字檔當作要求內文來傳遞。

您最多可以新增 9 萬個 OOV 字組，所有來源合計最多 1 千萬個字組，其中包括來自語料庫和文法的字組，以及您直接新增的字組。如需相關資訊，請參閱[我需要多少資料？](/docs/services/speech-to-text/language-resource.html#wordsResourceAmount)
{: note}
...
此方法為非同步。大約需要一、兩分鐘才會完成。作業的持續時間取決於語料庫中的字組總數、服務在語料庫中找到的新字組數目，以及服務的現行負載。如需檢查語料庫狀態的相關資訊，請參閱[監視新增語料庫要求](#monitorCorpus)。

您可以對每個語料庫文字檔各呼叫一次此方法，將任意數目的語料庫新增至自訂模型。必須徹底完成一個語料庫的新增之後，才能再新增另一個語料庫。將語料庫新增至自訂模型之後，請檢查新的自訂字組以找出拼字和其他錯誤。如需相關資訊，請參閱[驗證字組資源](/docs/services/speech-to-text/language-resource.html#validateModel)。
...
使用迴圈，每隔 10 秒檢查一次語料庫的狀態，直到它變成 `analyzed` 為止。如需檢查模型語料庫狀態的相關資訊，請參閱[列出自訂語言模型的語料庫](/docs/services/speech-to-text/language-corpora.html#listCorpora)。
...
-   如需直接新增字組的相關資訊，請參閱[使用自訂字組](/docs/services/speech-to-text/language-resource.html#workingWords)。
-   如需服務如何將自訂字組新增至模型的相關資訊，請參閱[新增或修改自訂字組時會發生什麼情況？](/docs/services/speech-to-text/language-resource.html#parseWord)

您可以使用下列方法將字組新增至自訂模型：

-   `POST /v1/customizations/{customization_id}/words` 方法可一次新增多個字組。您可以透過要求的內文來傳遞 JSON 物件，以提供每個字組的相關資訊。下列範例會將兩個自訂字組 `HHonors` 和 `IEEE` 新增至具有指定 ID 的自訂模型。`Content-Type` 標頭指定將 JSON 資料傳遞給方法。
...
如同新增語料庫，請檢查新的自訂字組以找出拼字和其他錯誤。當您一次新增多個字組時，此檢查尤其重要。如需相關資訊，請參閱[驗證字組資源](/docs/services/speech-to-text/language-resource.html#validateModel)。
...
在將新字組移入自訂語言模型（藉由新增語料庫、新增文法或直接新增字組）之後，您必須使用新資料來訓練模型。訓練會準備要將資料用於語音辨識的自訂模型。在使用那些資料來訓練模型之前，該模型不會使用您透過任何方法新增的字組。
...
您可以使用選用的 `word_type_to_add` 查詢參數來指定要用來訓練自訂模型的字組：

-   指定 `all` 或省略此參數會使用其所有字組來訓練模型，不論字組來源為何。
-   指定 `user` 只會使用使用者新增或修改的字組來訓練模型，而忽略只從語料庫或文法中擷取的字組。

    如果您新增的語料庫含有混雜資料（例如包含拼字錯誤的字組），這個選項會很有幫助。在使用這類資料訓練模型之前，您可以使用 `GET /v1/customizations/{customization_id}/words` 方法的 `word_type` 查詢參數，以檢閱從語料庫和文法中擷取的字組。如需相關資訊，請參閱[列出自訂語言模型中的字組](/docs/services/speech-to-text/language-words.html#listWords)。

此外，您還可以使用選用的 `customization_weight` 查詢參數。將自訂模型用於語音辨識時，此參數指定的相對權重是提供給來自自訂模型的字組，而不是來自基礎詞彙的字組。您也可以隨著使用自訂模型的任何辨識要求來指定自訂權重。如需相關資訊，請參閱[使用自訂權重](/docs/services/speech-to-text/language-use.html#weight)。

此方法為非同步。訓練需要幾分鐘才能完成，取決於用來訓練模型的新字組數目和服務的現行負載。如需檢查訓練作業狀態的相關資訊，請參閱[監視訓練模型要求](#monitorTraining-language)。
...
回應包含 `status` 和 `progress` 欄位，用來報告自訂模型的狀態。`progress` 欄位的意義取決於模型的狀態。`status` 欄位可能具有下列其中一個值：

-   `pending` 表示已建立模型，但正在等待新增訓練資料，或是等待服務完成所新增資料的分析。`progress` 欄位為 `0`。
-   `ready` 表示模型已準備好進行訓練。`progress` 欄位為 `0`。
-   `training` 表示正在訓練模型。訓練完成時，`progress` 欄位會從 `0` 變更為 `100`。<!-- The `progress` field indicates the progress of the training as a percentage complete. -->
-   `available` 表示模型已經過訓練，並備妥可供使用。`progress` 欄位為 `100`。
-   `upgrading` 表示正在升級模型。`progress` 欄位為 `0`。
-   `failed` 表示訓練模型失敗。`progress` 欄位為 `0`。

使用迴圈，每隔 10 秒檢查一次狀態，直到它變成 `available` 為止。如需檢查自訂模型狀態的相關資訊，請參閱[列出自訂語言模型](/docs/services/speech-to-text/language-models.html#listModels-language)。

### 訓練失敗
{: #failedTraining-language}

如果服務正在處理自訂語言模型的另一個要求，則訓練無法開始。例如，如果服務為下列情況，則訓練要求無法開始：
...
-   從自訂模型建立或前次訓練之後，未將任何訓練資料（語料庫、文法或字組）新增至自訂模型。
-   新增至自訂模型的一個以上字組具有無效的類似音發音，必須予以修正。

如果自訂模型的訓練狀態為 `failed`，請使用自訂作業介面的方法來檢查模型的字組，並修正您發現的所有錯誤。如需相關資訊，請參閱[驗證字組資源](/docs/services/speech-to-text/language-resource.html#validateModel)。
...
如需程式庫的相關資訊，請參閱 [pypi.python.org/pypi/requests ![外部鏈結圖示](../../icons/launch-glyph.svg "外部鏈結圖示")](https://pypi.python.org/pypi/requests){: new_window}。
1.  編輯 Script，以將 `password` 字串 `iam_apikey` 取代為 {{site.data.keyword.speechtotextshort}} 服務認證中的 API 金鑰：
...
1.  下載語料庫範例文字檔來與 Script 搭配使用。您可以使用其中一個語料庫文字檔或使用自己選擇的檔案來進行測試。依預設，所有語料庫文字檔都必須位於 Script 所在的相同目錄中。
1.  Script 使用 `curl` 指令來處理傳送至服務的 HTTP 要求。如果您尚未下載 `curl`，可以從 [curl.haxx.se ![外部鏈結圖示](../../icons/launch-glyph.svg "外部鏈結圖示")](http://curl.haxx.se){: new_window} 安裝您作業系統適用的版本。請安裝支援 Secure Sockets Layer (SSL) 通訊協定的版本，且務必在 `PATH` 環境變數中包含已安裝的二進位檔。
1.  編輯 Script，以將 `PASSWORD` 字串 `iam_apikey` 取代為 {{site.data.keyword.speechtotextshort}} 服務認證中的 API 金鑰：
