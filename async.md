...
The following example requests the status of the latest current jobs associated with the caller's service credentials. The user has three outstanding jobs in various states; the first job was created with a callback URL and a user token.
